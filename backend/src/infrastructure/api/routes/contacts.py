...
def list_contacts(db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
...
def get_contact(contact_id: int, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
    repository = SqlAlchemyContactRepository(db)
    use_case = GetContactUseCase(repository)
    contact = use_case.execute(contact_id, workspace_id)
    if not contact:
...
def create_contact(contact_dto: ContactCreateDTO, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id), team_id: Optional[int] = Depends(get_team_id_optional)):
    repository = SqlAlchemyContactRepository(db)
    use_case = CreateContactUseCase(repository)
    return use_case.execute(contact_dto, workspace_id, team_id)

@router.put("/{contact_id}", response_model=ContactReadDTO)
def update_contact(contact_id: int, contact_dto: ContactUpdateDTO, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
    repository = SqlAlchemyContactRepository(db)
    use_case = UpdateContactUseCase(repository)
    updated_contact = use_case.execute(contact_id, contact_dto, workspace_id)
    if not updated_contact:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contato não encontrado")
    return updated_contact

@router.delete("/{contact_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_contact(contact_id: int, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
    repository = SqlAlchemyContactRepository(db)
    use_case = DeleteContactUseCase(repository)
    success = use_case.execute(contact_id, workspace_id)
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Contato não encontrado")
    return None

@router.post("/{contact_id}/companies/{company_id}")
def link_company(contact_id: int, company_id: int, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
    repository = SqlAlchemyContactRepository(db)
...
def unlink_company(contact_id: int, company_id: int, db: Session = Depends(get_db), workspace_id: int = Depends(get_workspace_id)):
    repository = SqlAlchemyContactRepository(db)
