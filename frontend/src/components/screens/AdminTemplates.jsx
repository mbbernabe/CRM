        ...
        <div className="field-footer">
          <div className="field-checks">
            ...
            <label className="hs-checkbox field-required-check">
                ...
                />
                <span>Importação Obrigatória</span>
            </label>
          </div>
...
const AdminTemplates = () => {
  ...
  const [activeModalTab, setActiveModalTab] = useState('fields'); // 'fields' | 'table' | 'pipelines'
  ...
  const [visibleColumns, setVisibleColumns] = useState(['label', 'field_type', 'group', 'required', 'is_default', 'actions']);
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      ...
      color: '#0091ae',
      field_definitions: [
        { name: 'priority', label: 'Prioridade', field_type: 'select', options: ['Baixa', 'Média', 'Alta'], required: false, is_default: true, order: 0 }
      ],
      field_groups: []
    });
  ...
  const handleLibBulkDelete = async () => {
    ...
    try {
        const results = await Promise.all(
            libSelectedIds.map(id => fetchWithAuth(`/admin/templates/${id}`, { method: 'DELETE' }))
        );
  ...
  }, [isResizing, columnWidths, libColumnWidths, pipeColumnWidths]);
  ...
  const handleFieldsImported = (importedFields) => {
    ...
    const finalFields = importedFields.map((f, i) => {
        ...
        return {
            ...
            required: false,
            is_default: false, // Regra solicitada pelo usuário: não marcar como padrão ao importar CSV
            options: []
        };
    });
                                        ...
                                        <div className="column-picker-dropdown animate-in">
                                            ...
                                            <div className="picker-options">
                                                {[
                                                    ...
                                                    { key: 'is_default', label: 'Importação Obrigatória' }
                                                ].map(col => (
                                ...
                                <tbody>
                                    {filteredAndSortedFields.map((field) => (
                                        <tr key={field.originalIndex} className={selectedIndices.includes(field.originalIndex) ? 'row-selected' : ''}>
                                            ...
                                            {visibleColumns.includes('is_default') && (
                                                <td style={{ textAlign: 'center' }}>
                                                    <input 
                                                        type="checkbox" 
                                                        checked={field.is_default !== false} 
                                                        onChange={e => handleFieldChange(field.originalIndex, 'is_default', e.target.checked)}
                                                    />
                                                </td>
                                            )}
